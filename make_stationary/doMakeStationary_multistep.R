...
#####################################################################
### Calculate annualcycle and climate change signal multiple gam fits
#####################################################################

###############################################################################################
### step 1: fit gam mixed model (gamm) to get mean gmst term and baseline annual cycle
###############################################################################################
fm1.gamm    <- list(x ~ s(sdoy, bs="cc", k=12) +gmst, ~ 1 )
ft1.gamm    <- gamm(   fm1.gamm[[1]],    data=data01 )
 q1.gamm    <- predict(ft1.gamm$gam,  newdata=data01 ) 
# remove fit1 from data
data01$q1.gamm <- q1.gamm
data01$x1      <- data01$x0 - q1.gamm


###############################################################################################
### step 2: fit qgam to  residuals from step 1 ################################################
###############################################################################################
fm2.qgam <- list( x1 ~ s(stime, bs="tp", k=128), ~ 1 )
ft2.qgam <- qgam(   fm2.qgam[[1]], data=data01, qu=0.5 )
 q2.qgam <- predict(ft2.qgam    newdata=data01, type="response" )

## cacluate second order residuals
data01$q2.qgam <- q2.qgam
data01$x2      <- data01$x1 - q2.qgam


GOT TO HERE
need to think carefuly the end goal and what makes sense.

...
##############################################################################
fn_fit_qgam <- function(fmla.qgam, data01, ptiles=(1:99)/100) {
    ...
    print(qdo(fit.qgam, max(ptiles), summary))
...
##############################################################################
fn_apply_q2p <- function(x, q2p, st.q2p=NULL, SANITY=FALSE) {
...
fn_fit_MSgpd <- function(data01, ms.thgpd.u, fmla.MSgpd, chosen.MSgpd.name=NULL, fit.qgam=NULL, st.qgam=NULL, st.msgpd=NULL) {
    ...
    data01$excess     <- data01$residStep1 - th.01.gpd
    ...
    if(is.null(chosen.MSgpd.name)) {
        ...
        fit.MSgpd      <- fits.MSgpd[[ix1]]

        cat('Chosen MS GPD:',chosen.MSgpd.name,cr)
        print(summary(fit.MSgpd))

        fit.MSgpd$ms.thgpd.u <- ms.thgpd.u
        date01 <- data01[,c('time','doy')]
        save(file=st.msgpd, fits.MSgpd, fit.MSgpd, chosen.MSgpd.name, th.01.gpd, date01)

    } else {
        cat("Fitting GPD",chosen.MSgpd.name,cr)
        # sink("/dev/null")
        fit.MSgpd      <- evgam(fmla.MSgpd[[chosen.MSgpd.name]], data.gpd, family="gpd", trace=2)
        # sink()
        ...
        fit.MSgpd$ms.thgpd.u <- ms.thgpd.u
        fit.MSgpd$fn.thgpd   <- qdo(fit.qgam, ms.thgpd.u)
        date01 <- data01[,c('time','doy')]
        save(file=st.msgpd, fit.MSgpd, chosen.MSgpd.name, th.01.gpd, date01)
    }
...
fn_apply_MSgpd <- function(data01, st.msgpd=NULL, QUIET=TRUE) {
    ...
    ms.thgpd.u <- fit.MSgpd$ms.thgpd.u
    ...
    th.gpd        <- predict(fit.MSgpd$fn.thgpd, newdata=data01)
    ...
    gpd01par <- predict(fit.MSgpd, newdata=data01, type="response")
    u01      <- data01$uqgam
    excess   <- data01$residStep1 - th.gpd # data01$excess
